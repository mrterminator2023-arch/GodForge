...
[ -f "$ROOT/bin/Release/net8.0/GodForge.dll" ] || {
  echo "GodForge.dll not built, building..." >&2
  "$DOTNET" build "$ROOT/GodForge.csproj" -c Release -p:SkipNmlPostBuild=true -v q
}
...
"$DOTNET" build "$ROOT/tools/mod_template/Mod.csproj" -c Release -v q -nologo \
  -p:ModDir="$MOD_DIR" -p:ModGuid="$GUID" -p:GodForgeRoot="$ROOT" -p:Il2CppDir="$IL2CPP_DIR" \
