...
/// <summary>
/// Roslyn-backed compiler for source mods. Everything that touches Microsoft.CodeAnalysis lives here so that
/// this type is only loaded (and Roslyn only resolved) when a mod actually needs compiling and the optional
/// compiler pack (<see cref="Paths.CompilerPackPath"/>) has been loaded via <see cref="CompilerPack"/>.
/// </summary>
internal static class ModCompiler
{
    ...
    /// </summary>
    internal static void PrepareReferences(List<ModDependencyNode> pModNodes)
    {
    ...
    internal static bool IsPrepared => _default_ref != null;

    /// <summary>
    /// Build the default reference set (game, MelonLoader, Il2Cpp assemblies, WBML itself). Idempotent.
    /// </summary>
    internal static void EnsureDefaultReferences()
    {
    ...
    /// Prepare references for a single mod node
    /// </summary>
    internal static void PrepareRuntime(ModDependencyNode pModNode)
    {
    ...
    internal static bool CompileNode(ModDependencyNode pModNode, bool pForce = false)
    {
        EnsureDefaultReferences();
        bool compile_result;
